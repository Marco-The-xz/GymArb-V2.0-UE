...
namespace XRPassthrough
{
	...
	void FPassthroughXR::ShutdownPassthrough(XrSession InSession)
	{
		...
		([this,
			CapturedLayers = MoveTemp(TmpLayers),
			CapturedMeshes = MoveTemp(TmpMeshes),
			PassthroughHandle](FRHICommandListImmediate& RHICmdList) mutable {
			PokeAHolesToRender_RenderThread.Reset();

			RHICmdList.EnqueueLambda([this, 
				LayersToDestroy = MoveTemp(CapturedLayers),
				MeshesToDestroy = MoveTemp(CapturedMeshes),
				PassthroughHandle](FRHICommandListImmediate& RHICmdList) mutable {
				UnderlayLayers_RHIThread.Reset();
				OverlayLayers_RHIThread.Reset();
				TransformsToUpdate_RHIThread.Reset();

				for (const FPassthroughLayer& Layer : LayersToDestroy)
				{
				...
				for (const FUserDefinedMesh& Mesh : MeshesToDestroy)
				{
